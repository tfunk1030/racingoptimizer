...
| §5.3 lap-time Spearman | ρ ≥ 0.30 per qualifying (car, track) pair | **FIRST MEASUREMENT (2026-06-10): FAIL** — cadillac @ lagunaseca ρ = **−0.067** (12 LOSO pairs): no detectable score↔lap-time correlation. Caveats: n=12 is underpowered (95 % CI ≈ ±0.58, cannot confirm *or* exclude 0.30); Cadillac is the H2 worst case (aero scored at the clamped 25 mm floor); and the LOSO refit uses only same-track sessions (`lap_time_correlation_gate.py:260` `rest` ⊂ pair sessions) despite its comment claiming cross-track variety — production models pool all tracks, so this measures a thinner fit than production. First execution also crash-fixed a latent `SessionRow.valid` bug. Results: `docs/physics-rebuild/lap_time_correlation_latest.json` | `scripts/lap_time_correlation_gate.py:31,161-380` |
