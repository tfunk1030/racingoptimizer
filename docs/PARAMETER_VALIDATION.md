...
> **Currency (2026-05-23):** corner weights are READOUT-only in ontology
> (not BLOCKED). Rear `SpringPerchOffset` is mirrored LR→RR in the setup
> card (`_MIRRORED_LEAVES`); unmapped RR perch rows in per-car tables below
> are expected — the optimizer writes `[OPT]` on LeftRear and `[OPT mirror]`
> on RightRear. Re-run `scripts/validate_ontology_paths.py` to refresh counts.
